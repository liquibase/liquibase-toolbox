#!/bin/bash
VERSION="$(curl  -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/liquibase/liquibase/releases/latest | grep -o "v.*" | grep -v "\"\," | grep "jar" | sed s/"v"//g | sed s/"\/.*"//g)"
