...
liquibase --url=offline:$databaseType --changeLogFile=$referenceChangeLog updateSQL > /dev/null
liquibase --url=offline:$databaseType --changeLogFile=$referenceChangeLog rollbackCountSQL 9999 | grep -iv DATABASECHANGELOG.* | sed -r 's/::/:/g' | sed -r 's/Rolling Back ChangeSet:/changeset/g' | sed -r "s/${referenceChangeLog}://g" >> $targetChangeLog
